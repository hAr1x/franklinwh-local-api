...
"""
test_battery_charge.py - Verification test for async_start_battery_charge().

Tests the M704 manual battery charge command path end-to-end.
...
def _print_status_summary(label: str, status) -> None:
    ...
    print(f"  battery_command_charge_w:{status.battery_command_charge_w:.1f} W")
...
async def main() -> None:
    ...
    try:
        ...
        _line(f"STEP 2: async_start_battery_charge(power_w={TEST_POWER_W}, duration_s={TEST_DURATION_S})")
        try:
            await client.async_start_battery_charge(power_w=TEST_POWER_W, duration_s=TEST_DURATION_S)
        ...
        print(f"  (expected wset_pct approx {expected_pct:.1f}%; negative = charging)")

        _line(f"STEP 4: Sampling every {OBSERVE_WAIT_S}s for {OBSERVE_DURATION_S}s")
        n_samples = OBSERVE_DURATION_S // OBSERVE_WAIT_S
        print(f"{'t(s)':>6}  {'batt_W':>9}  {'batt_chg_W':>11}  {'grid_W':>9}  {'grid_imp_W':>11}  {'cmd_pct_raw':>11}")
        for i in range(1, n_samples + 1):
            await asyncio.sleep(OBSERVE_WAIT_S)
            elapsed = i * OBSERVE_WAIT_S
            status_i = await client.async_get_status()
            print(f"{elapsed:>6}  {status_i.battery_power_w:>9.0f}  {status_i.battery_charging_w:>11.0f}  {status_i.grid_power_w:>9.0f}  {status_i.grid_import_w:>11.0f}  {status_i.battery_command_pct_raw:>11}")
        status1 = status_i
        print()
